...
function applySettings() {
    ...
    if (MINISCROLLPLAYER) {enableMiniPlayer()}
...
let cancelMiniPlayerWait;
function enableMiniPlayer() {
    createminiscrollplayercss()
    cancelMiniPlayerWait = Wait('#player', (e) => {
        const start = $(e).offset().top;
        const end = start + $(e).outerHeight(); //start + 600;
        $(window)
            .off("scroll.gpMiniPlayer")
            ...
            $(e)[0].style.setProperty("--progress", progress);
        });
    });
}
