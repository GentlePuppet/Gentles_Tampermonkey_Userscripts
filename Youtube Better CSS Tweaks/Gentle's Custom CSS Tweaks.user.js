...
const gmc = new GM_config({
    ...
    fields: {
        FORCE1080: {
            ...
            type: 'checkbox',
            default: true,
            title: 'Changes video Quality to 1080p when the video starts playing.'
        },
        CONVERTNERDS: {
            label: 'Convert Stats for Nerds',
            type: 'checkbox',
            default: true,
            title: "Kbps to KB/s in Stat's For Nerds"
        },
        ROBOTOCONDENSED: {
            label: 'Roboto Condense Override',
            type: 'checkbox',
            default: true,
    ...
    css: `
         ...
         #GentlesCustomCSSTweaks_config_wrapper {background: #211a28; padding: 18px; border: 1px solid #9c51e7; border-radius: 10px;}
         #GentlesCustomCSSTweaks_config {background: transparent; color: #e6e6e6;}
         /* Headings */
         ...
         /* Section */
         #GentlesCustomCSSTweaks_config .section_header_holder {margin-top: 18px;}
         ...
         /* Labels */
         #GentlesCustomCSSTweaks_config .field_label {color: #ffffff; font-size: 13px; font-weight: 600; margin-right: 8px;}
         ...
         #GentlesCustomCSSTweaks_config_buttons_holder {color: #9bbcff; text-align: right; font-size: 12px;}
         ...
         /* Misc */
         #GentlesCustomCSSTweaks_config .config_var {margin: 10px 0;}
...
onInit(gmc).then(() => {
    const FORCE1080 = gmc.get("FORCE1080");
...
/* Smaller Sidebar */
   * {--sidebar-width: 160px}
   ...
   #contentContainer.tp-yt-app-drawer[opened], tp-yt-app-drawer[opened], #guide-inner-content.ytd-app, ytd-guide-renderer.ytd-app, ytd-guide-entry-renderer {width: var(--sidebar-width) !important;}
...
function Wait(selector, callback, once = 0) {return waitForKeyElements(selector, callback, once);}
    ...
    // Make the sorting playlist page super tiny and easy to sort for big lists.
    if (gmc.get('TINYPLAYLISTPAGE')) {
        window.addEventListener("yt-page-data-updated", function(e) {
            ...
            const playlistInterval = setInterval(() => {
                if(window.location.href.indexOf("playlist?list=")!==-1){t()}
                else{clearInterval(playlistInterval)}
            }, 250);
        })
    }

    // Change the font of everything to Roboto Condense (I like it)
    if (gmc.get('ROBOTOCONDENSED')) {
        var fontoverride = $('<style class="fontfamily">* {font-family: "Roboto Condensed" !important;}</style>');
        var fontlink = $('<link class="fontlink" rel="stylesheet" href="//fonts.googleapis.com/css2?family=Roboto+Condensed">');
    ...
    if (gmc.get('TALLERPLAYLIST')) {
        ...
        Wait('#container.ytd-playlist-panel-renderer, ytd-playlist-panel-renderer[js-panel-height_]', playlistHeight);
        Wait('ytd-watch-flexy[flexy_][js-panel-height_] #panels.ytd-watch-flexy ytd-engagement-panel-section-list-renderer.ytd-watch-flexy', playlistHeight);
    ...
    window.addEventListener("yt-page-data-updated", function(e) {
        ...
        // Replace X in playlist header with Up Arrow
        ...
        // Remove "MORE FROM YOUTUBE" Section from Left Sidebar
        Wait('a[title="YouTube Premium"]', function(e) {e.parents('ytd-guide-section-renderer').remove();});

        // Remove Tooltip from Video Titles
        Wait('#video-title, .ytLockupMetadataViewModelHeadingReset', function(e) {var titleattr = e.attr('title');e.attr('data-title', titleattr);e.removeAttr('title');});

        // Remove A Bunch Of Things
        Wait(`
	    yt-lockup-view-model:has(.ytDismissibleItemReplacedContent),
        ...
        yt-button-view-model:has(path[d*="M480-80q0-83-31.5-156T363-363q-54-54-127-85.5T80-480q83"]),
        [aria-label="Ask about this video"],
        #footer.ytd-guide-renderer,
        how-this-was-made-section-view-model,
        ytd-video-description-infocards-section-renderer[show-creator-custom-url-buttons],
        yt-video-attributes-section-view-model:has(.videoAttributesSectionViewModelTitleContainer .videoAttributesSectionViewModelTitle:contains("Games")),
        yt-video-attributes-section-view-model:has(.videoAttributesSectionViewModelHeader .videoAttributesSectionViewModelTitleContainer h3.videoAttributesSectionViewModelTitle),
        #clarify-box,
        #donation-shelf,
        #offer-module,
        #video-summary,
        #merch-shelf,
        #ticket-shelf,
        #header #title:contains("Transcript"),
        #sub-header #sub-header-text:contains("Follow along using the transcript."),
        #teaser-carousel,
        ytd-menu-service-item-download-renderer.ytd-menu-popup-renderer[role="menuitem"]
	`, (e) => {$(e).remove()});
        ...
        a[title="Your videos"]
    `, (e) => {$(e).parents('ytd-guide-entry-renderer').remove()});

        // Remove The Explore and More from Youtube Sections
        Wait(`
        .ytd-guide-section-renderer:contains(Explore),
        .ytd-guide-section-renderer:contains(More from YouTube),
        a.ytd-guide-entry-renderer[title="Settings"]
    `, (e) => {$(e).parents('ytd-guide-section-renderer').remove()});

        // Remove Different types of Videos
        Wait(`
        yt-formatted-string.ytd-thumbnail-overlay-side-panel-renderer:contains("Mix"),
        ytd-toggle-button-renderer #button.ytd-toggle-button-renderer:contains("Set reminder"),
        .yt-core-attributed-string--white-space-no-wrap:contains("Notify me"),
        #video-title.style-scope.ytd-compact-radio-renderer[title*="Mix - "],
        ytd-channel-name > div > div > yt-formatted-string:contains("Youtube"),
        #metadata > div > span:contains("Premieres"),
        span:contains("LIVE NOW"),
        [class*="Button"]:contains("Notify me"),
        .ytd-badge-supported-renderer:contains("LIVE"),
        h3[aria-label*="by YouTube Movies"],
        ytd-compact-video-renderer > #dismissible,
        [overlay-style=SHORTS],
        .ytAttributedStringHost:contains("Get answers, explore topics, and more"),
        .ytBadgeShapeText:contains("Members only"),
        .ytBadgeShapeText:contains("Buy or rent"),
        .ytBadgeShapeText:contains("Buy"),
        .ytBadgeShapeText:contains("Try now"),
        [class*="yt-badge"]:contains("Members only")
    `, (e) => {
            const selectors = [
        ...
        if (gmc.get('HIDENEWTAG')) {
            Wait('.ytBadgeShapeText:contains(New)', function(e) {e.parents('.ytContentMetadataViewModelMetadataRow').remove();});
        }
        ...
        if (gmc.get('FORCE1080')) {
            function applyQuality() {
                ...
                if (levels.includes('hd1080') && quality !== 'hd1080') {
                    player.setPlaybackQualityRange('hd1080','1080p60 Premium');
                    player.setPlaybackQuality('hd1080','1080p60 Premium');
