...
// @require      https://raw.githubusercontent.com/GentlePuppet/waitForElement/refs/heads/main/waitForElementCancel.js
...
Wait.cancel = function(selector) {return waitForKeyElements.cancel(selector);};

// General CSS Overrides
{const stylesheet = `
/* Shrinkable Player CSS */
body.gp-mini-scroll-player #player {
    ...
    transform:
        translate(
            calc(1px * var(--progress)),
            calc(66px * var(--progress))
        )
        scale(calc(1 - .7 * var(--progress)));
}
...
/* Better Sub Page */
    ...
    [page-subtype="subscriptions"] .ytContentMetadataViewModelMetadataRow {flex-wrap: wrap;}
    [page-subtype="subscriptions"] .ytContentMetadataViewModelMetadataText:has([href]) {flex-basis: 100%;}
...
/* Wider Video - Smaller Recommended */
    ...
    /*  #player-container-outer {max-width: 97% !important;min-width: 97% !important;} */
    #player-container-outer, .html5-video-player:not(div:has(.video-stream[style*="width: 400px"])):not(.ytp-fullscreen), .html5-video-player:not(.ytp-fullscreen) .html5-main-video:not([style*="width: 400px"]), #container.ytd-player:not(:has(.ytp-fullscreen)), ytd-player:not(:has(.ytp-fullscreen)) {
    ...
    ytd-watch-flexy[split-scroll]:not([fixed-panel-watch-next])[fixed-panel-expanded][is-extra-wide-video_] #secondary-split-scroll-spacer.ytd-watch-flexy, ytd-watch-flexy[split-scroll]:not([fixed-panel-watch-next])[fixed-panel-expanded][is-four-three-to-sixteen-nine-video_] #secondary-split-scroll-spacer.ytd-watch-flexy, ytd-watch-flexy[split-scroll][fixed-panel-watch-next][is-extra-wide-video_] #secondary-split-scroll-spacer.ytd-watch-flexy, ytd-watch-flexy[split-scroll][fixed-panel-watch-next][is-four-three-to-sixteen-nine-video_] #secondary-split-scroll-spacer.ytd-watch-flexy {
        max-width: 80%;
        min-width: 80%;
    }
...
/* - Rearrange Playlist to Make it More Compact - */
 /* - Playlist Header - */
    ...
    div#header-description.ytd-playlist-panel-renderer {order: 2; margin: 0px 5px 0px 5px; justify-items: baseline;}
    div#header-description.ytd-playlist-panel-renderer, #next-video-title yt-formatted-string[ellipsis-truncate-styling] {max-width: 50%;}
 ...
 /* - Playlist Items Menu Button - */
    #menu.ytd-playlist-panel-video-renderer {z-index: 999; position: relative; opacity: 0; right: 50px; align-self: center;}
...
/* Purple Dark Theme */
 ...
 /* - Buttons - */
    ...
    .ytLockupMetadataViewModelMoveLockupOverflowMenuToBottomRight .ytLockupMetadataViewModelMenuButton:after {display: none;}
 ...
 /* - Anything with borders - */
  ...
  /* - Other Purple Dark Theme Things | Uncommented - */
    .ytContextualSheetLayoutHost, .ytdMiniplayerInfoBarHost, .header.ytd-playlist-panel-renderer, .playlist-items.ytd-playlist-panel-renderer {background-color: var(--gp-black-purple) !important;}
...
`;
    ...
    styleTag.classList.add("userscriptstyle");
    styleTag.id = "Gentles-Custom-YouTube-CSS";
    styleTag.textContent = stylesheet;
    document.body.insertAdjacentElement('afterend', styleTag);
}
...
const gmc = new GM_config({
    ...
    fields: {
        ...
        MINISCROLLPLAYER: {
            label: 'Shrink player as you scroll',
            type: 'checkbox',
            default: true,
            title: 'Shrinks and keeps the player visible while you scroll down.'
        },
        MINISCROLLPLAYERLEFT: {
            label: 'Place Mini Scroll Player on Left',
            type: 'checkbox',
            default: true,
            title: 'Turning this off moves the player to the right instead.'
        },
        HIDEJOINMEMBERS: {
            label: 'Hide "Join" members button',
            type: 'checkbox',
            default: true,
            title: 'Hides the join button to become a channel member.'
        },
        FORCEH264: {
...
/* Section */
#GentlesCustomCSSTweaks_config .section_header_holder {margin-top: 18px; margin-bottom: 10px; overflow-x: hidden; overflow-y: scroll; height: 70%; border: 1px solid #9c51e7; border-radius: 10px;}
...
function createminiscrollplayercss() {
        document.querySelector('#MINISCROLLPLAYERCSS')?.remove()
        if (gmc.get('MINISCROLLPLAYERLEFT') && !document.querySelector('#MINISCROLLPLAYERCSS')) {
            // Left Enabled
            document.body.insertAdjacentHTML('afterend', '<style class="userscriptstyle" id="MINISCROLLPLAYERCSS"> body.gp-mini-scroll-player #player {transform-origin: top left; left: 12px;}</style>');
        } else {
            // Left Disabled => Right Enabled
            document.body.insertAdjacentHTML('afterend', '<style class="userscriptstyle" id="MINISCROLLPLAYERCSS"> body.gp-mini-scroll-player #player {transform-origin: top right; right: 12px;}</style>');
        }
}

function miniscrollplayer() {
    const MINISCROLLPLAYER = gmc.get("MINISCROLLPLAYER");
    if (MINISCROLLPLAYER) {
        createminiscrollplayercss()
        Wait('[is-watch-page] #player', (e) => {
            const start = $(e).offset().top;
            const end = start + $(e).outerHeight();
            $(window).off("scroll.gpMiniPlayer")
                .on("scroll.gpMiniPlayer", function () {
                if ($(this).scrollTop() > start) {$("body").addClass("gp-mini-scroll-player");} else {$("body").removeClass("gp-mini-scroll-player");}
                const scroll = $(this).scrollTop();
                let t = (scroll - start) / (end - start);
                t = Math.max(0, Math.min(1, t));
                t = 1 - Math.pow(1 - t, 3);
                const progress = Math.max(0, Math.min(1, t));
                $(e).css("--progress", progress);
            });
        });
    } else {
        Wait.cancel('#player')
        $('#player').removeData('alreadyFound');
        $("body").removeClass("gp-mini-scroll-player");
        $(window).off("scroll.gpMiniPlayer");
        document.querySelector('#MINISCROLLPLAYERCSS')?.remove()
    }
}
...
onInit(gmc).then(() => {
    ...
    const MINISCROLLPLAYER = gmc.get("MINISCROLLPLAYER");
    const MINISCROLLPLAYERLEFT = gmc.get("MINISCROLLPLAYERLEFT");
    ...
    const HIDEJOINMEMBERS = gmc.get("HIDEJOINMEMBERS")
...
window.addEventListener("yt-page-data-updated", function(e) {
    ...
    if (AUTOAGREE) {
        Wait('[aria-label="Viewer discretion is advised"] button-view-model > a', (e) => {$(e).click()[0];});
    }

    // Replace the shitty new views metadata info with the old one
    // Remove Recommended Videos With Under 2000 Views
    if (HIDE1000VIDEOS) {
        Wait('.ytAttributedStringHost.ytContentMetadataViewModelMetadataText[aria-label*="views"]', (e) => {
            if (!$(e).text().includes(' views')) {
                ...
                if (value < 1000) {$(e).parents('yt-lockup-view-model.ytd-item-section-renderer').remove();}
                if (text.toLowerCase() == "no views") {$(e).parents('yt-lockup-view-model.ytd-item-section-renderer').remove();}
            }
        });
    } else {
        Wait('.ytAttributedStringHost.ytContentMetadataViewModelMetadataText[aria-label*="views"]', (e) => {
    ...
    if (HIDEJOINMEMBERS) {
        Wait('#sponsor-button', (e) => {$(e).remove()})
    }
    ...
    if (!document.querySelector('.ScrollToTopContainer')) {
        const stylesheet = `
            .scrolltotop {z-index: 999999; position: fixed; bottom: 30px; left: 20px; width: 40px; height: 40px; border: 1px solid var(--gp-dark-purple); border-radius: 50% !important; background: var(--gp-lightdark-purple); color: white; font-size: 20px; display: flex; align-items: center; justify-content: center; cursor: pointer; box-shadow: 0 0 18px rgba(0,0,0,1.2); transition: transform 0.2s ease;}
            .scrolltotop svg {width: 20px; height: 20px;}
            .scrolltotop:hover {transform: scale(1.1); background: var(--gp-header-purple-outline);}
        `;
        const styleTag = document.createElement('style');
        styleTag.classList.add("userscriptstyle");
        styleTag.id = "ScrollToTopCSS";
        styleTag.textContent = stylesheet;
        ...
        <svg xmlns="http://www.w3.org/2000/svg" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" image-rendering="optimizeQuality" fill-rule="evenodd" clip-rule="evenodd" viewBox="0 0 463.96 512">
            <path fill="currentColor" fill-rule="nonzero" d="M332.67 512V268.5h92.3c15.48-.68 26.47-5.77 32.82-15.42 17.21-25.8-5.25-52.31-22.6-69.25L261.61 14.33c-17.29-19.11-41.93-19.11-59.22 0L24.42 188.72C8.03 204.78-9.67 229.27 6.21 253.08c6.35 9.65 17.34 14.74 32.81 15.42h92.31V512h201.34z"/>
        </svg>
        `;
        ...
        btncontainer.className = "ScrollToTopContainer";
        document.body.insertAdjacentElement('afterend', btncontainer);
        btncontainer.appendChild(styleTag);
    ...
    // Remove A Bunch Of Things
    {
        ...
        Wait('button-view-model.you-chat-entrypoint-button:has([aria-label="Ask"])', REMOVETHING);                                   // Removes the Ask AI button from fullscreen
    ...
    {
        Wait('ytd-app[is-watch-page] yt-formatted-string.ytd-thumbnail-overlay-side-panel-renderer:contains("Mix")', RemoveSelectors);           // Removes Mix playlists
        Wait('ytd-app[is-watch-page] #video-title.style-scope.ytd-compact-radio-renderer[title*="Mix - "]', RemoveSelectors);                    // Removes Podcast playlists
        Wait('ytd-app[is-watch-page] .ytAttributedStringLinkCallToActionColor:contains("Podcast")', RemoveSelectors);                            // Removes Mix playlists
        Wait('ytd-app[is-watch-page] .ytAttributedStringLinkCallToActionColor:contains("Playlist")', RemoveSelectors);                           // Removes Random playlists
        Wait('ytd-app[is-watch-page] ytd-toggle-button-renderer #button.ytd-toggle-button-renderer:contains("Set reminder")', RemoveSelectors);  // Removes Premier videos
        Wait('ytd-app[is-watch-page] .yt-core-attributed-string--white-space-no-wrap:contains("Notify me")', RemoveSelectors);                   // Removes Premier videos
        Wait('ytd-app[is-watch-page] [class*="Button"]:contains("Notify me")', RemoveSelectors);                                                 // Removes Premier videos
        Wait('ytd-app[is-watch-page] #metadata > div > span:contains("Premieres")', RemoveSelectors);                                            // Removes Premier videos
        Wait('ytd-app[is-watch-page] ytd-channel-name > div > div > yt-formatted-string:contains("Youtube")', RemoveSelectors);                  // Removes Videos uploaded by Youtube
        Wait('ytd-app[is-watch-page] h3[aria-label*="by YouTube Movies"]', RemoveSelectors);                                                     // Removes Movies
        Wait('ytd-app[is-watch-page] span:contains("LIVE NOW")', RemoveSelectors);                                                               // Removes Live Videos
        Wait('ytd-app[is-watch-page] .ytd-badge-supported-renderer:contains("LIVE")', RemoveSelectors);                                          // Removes Live Videos
        Wait('ytd-app[is-watch-page] ytd-compact-video-renderer > #dismissible', RemoveSelectors);                                               // I don't remember what this removes
        Wait('[overlay-style=SHORTS]', RemoveSelectors);                                                                                         // Removes Shorts
        Wait('ytd-app[is-watch-page] .ytAttributedStringHost:contains("Get answers, explore topics, and more")', RemoveSelectors);               // Removes AI search (I think, I don't rmemeber)
        Wait('.ytBadgeShapeText:contains("Members only")', RemoveSelectors);                                                                     // Removes Members only videos
        Wait('ytd-app[is-watch-page] .ytBadgeShapeText:contains("Buy or rent")', RemoveSelectors);                                               // Removes Buy/Rent/Try Videos
        Wait('ytd-app[is-watch-page] .ytBadgeShapeText:contains("Buy")', RemoveSelectors);                                                       // Removes Buy/Rent/Try Videos
        Wait('ytd-app[is-watch-page] .ytBadgeShapeText:contains("Try now")', RemoveSelectors);                                                   // Removes Buy/Rent/Try Videos
