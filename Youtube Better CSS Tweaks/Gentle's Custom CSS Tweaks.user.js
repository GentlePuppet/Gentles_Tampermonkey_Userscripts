...
Wait('#description.ytd-watch-metadata', DESCBACKGROUNDOVERRIDE);
function DESCBACKGROUNDOVERRIDE(e) {$(e).attr('style', 'background: var(--gp-lightblack-purple)')}

// Video Title Hover Background Override
Wait('.ytSpecTouchFeedbackShapeHoverEffect', TITLEBACKGROUNDOVERRIDE);
Wait('.ytSpecTouchFeedbackShapeStroke', TITLEBACKGROUNDOVERRIDE);
Wait('.ytSpecTouchFeedbackShapeFill', TITLEBACKGROUNDOVERRIDE);
function TITLEBACKGROUNDOVERRIDE(e) {$(e).attr('style', 'background: var(--gp-dark-purple)')}
...
{
    Wait('yt-lockup-view-model:has(.ytDismissibleItemReplacedContent)', REMOVETHING);
    Wait('ytd-channel-renderer.ytd-item-section-renderer', REMOVETHING);
    Wait('.ytp-gradient-bottom', REMOVETHING);
    Wait('button[title="Miniplayer (i)"]', REMOVETHING);
    Wait('button[title="Picture in Picture"]', REMOVETHING);
    Wait('button[aria-label="Theater mode (t)"]', REMOVETHING);
    Wait('yt-related-chip-cloud-renderer', REMOVETHING);
    Wait('ytd-video-quality-promo-renderer', REMOVETHING);
    Wait('ytd-rich-shelf-renderer', REMOVETHING);
    Wait('ytd-reel-shelf-renderer', REMOVETHING);
    Wait('ytd-guide-downloads-entry-renderer', REMOVETHING);
    Wait('yt-button-view-model:has(path[d*="M480-80q0-83-31.5-156T363-363q-54-54-127-85.5T80-480q83"])', REMOVETHING);
    Wait('[aria-label="Ask about this video"]', REMOVETHING);
    Wait('#footer.ytd-guide-renderer', REMOVETHING);
    ...
    Wait('ytd-menu-service-item-download-renderer.ytd-menu-popup-renderer[role="menuitem"]', REMOVETHING);
    function REMOVETHING(e) {$(e).remove()};
}

// Remove Different types of Videos
{
    Wait('yt-formatted-string.ytd-thumbnail-overlay-side-panel-renderer:contains("Mix")', RemoveSelectors);
    Wait('ytd-toggle-button-renderer #button.ytd-toggle-button-renderer:contains("Set reminder")', RemoveSelectors);
    Wait('.yt-core-attributed-string--white-space-no-wrap:contains("Notify me")', RemoveSelectors);
    Wait('#video-title.style-scope.ytd-compact-radio-renderer[title*="Mix - "]', RemoveSelectors);
    Wait('ytd-channel-name > div > div > yt-formatted-string:contains("Youtube")', RemoveSelectors);
    Wait('#metadata > div > span:contains("Premieres")', RemoveSelectors);
    Wait('span:contains("LIVE NOW")', RemoveSelectors);
    Wait('[class*="Button"]:contains("Notify me")', RemoveSelectors);
    Wait('.ytd-badge-supported-renderer:contains("LIVE")', RemoveSelectors);
    Wait('h3[aria-label*="by YouTube Movies"]', RemoveSelectors);
    Wait('ytd-compact-video-renderer > #dismissible', RemoveSelectors);
    Wait('[overlay-style=SHORTS]', RemoveSelectors);
    Wait('.ytAttributedStringHost:contains("Get answers, explore topics, and more")', RemoveSelectors);
    Wait('.ytBadgeShapeText:contains("Members only")', RemoveSelectors);
    Wait('.ytBadgeShapeText:contains("Buy or rent")', RemoveSelectors);
    Wait('.ytBadgeShapeText:contains("Buy")', RemoveSelectors);
    Wait('.ytBadgeShapeText:contains("Try now")', RemoveSelectors);
    Wait('[class*="yt-badge"]:contains("Members only")', RemoveSelectors);
